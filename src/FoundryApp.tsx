...
export default function FoundryApp() {
  ...
  const updateSample = useCallback((sample: Sample) => setWorkspace(w => ({ ...w, samples: w.samples.map(s => s.id === sample.id ? sample : s) })), [])
  ...
  async function importFiles(files: FileList | File[]) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        newSamples.push({ id: crypto.randomUUID(), packId: pack.id, name: file.name, originalName: file.name, blob: file, category: analysis.category, tags: analysis.tags, bpm: analysis.bpm, key: analysis.key, analysis, status: 'review', favorite: false, createdAt: Date.now(), trim: false, normalize: false, fade: false })
      } catch { errors.push(`${file.name}: could not decode. Try a standard PCM WAV, MP3, FLAC or OGG file.`) }
      await new Promise(r => setTimeout(r, 10))
    }
    if (newSamples.length) { setWorkspace(w => ({ ...w, samples: [...newSamples, ...w.samples] })); switchView('pack'); notify(`${newSamples.length} ${newSamples.length === 1 ? 'sample' : 'samples'} imported. Audio measured; suggested metadata is ready to review.`) }
  ...
  async function identifySounds() {
    ...
    try {
      for (const id of analysisIds) {
        ...
        setWorkspace(w => ({ ...w, samples: w.samples.map(s => s.id === id ? { ...s, category: reliable ? categoryFromLabel(top.label) : s.category, tags: [...new Set([...s.tags, ...predictions.filter(p => p.score >= 0.3).map(p => p.label.toLowerCase())])], status: 'review', analysis: { ...s.analysis, method: 'model', predictions } } : s) }))
        count++
      }
  ...
  return <div className="app-shell" onDragOver={e => { if (e.dataTransfer.types.includes('Files')) e.preventDefault() }} onDrop={e => { if (e.dataTransfer.files.length) { e.preventDefault(); void importFiles(e.dataTransfer.files) } }}>
    ...
    <div className="main-shell">
      ...
      <main className="workspace-main">
        ...
        <section className="library-section">
          ...
          {selected.size > 0 && <div className="selection-toolbar"><span><CheckCheck size={15} />{selected.size} selected</span><button onClick={() => { setWorkspace(w => ({ ...w, samples: w.samples.map(s => selected.has(s.id) ? { ...s, status: 'ready' } : s) })); notify('Selected metadata confirmed. Samples marked ready.'); setSelected(new Set()) }}><Check size={14} />Mark ready</button><button onClick={() => openAnalysis()}><Sparkles size={14} />Identify</button><button onClick={() => { setBulkTags(''); setDialog('bulkTags') }}>Add tags</button><button className="delete-selection" onClick={() => setDialog('delete')} aria-label="Delete selected samples"><Trash2 size={15} /></button><button onClick={() => setSelected(new Set())} aria-label="Clear selection"><X size={15} /></button></div>}
    ...
    {dialog === 'analyze' && <Modal title="Every sound has a story." subtitle={`Identify ${analysisIds.length} ${analysisIds.length === 1 ? 'sample' : 'samples'} without sending your audio anywhere.`} onClose={closeDialog}><div className="analysis-step"><span className="step-icon done"><Check size={20} /></span><div><h3>Signal analysis</h3><p>Waveform, duration, sample rate, channels, peak and RMS levels are measured during import. Tempo and root-note estimates appear only when sufficiently periodic.</p><span className="step-state">Already measured · No model needed</span></div></div><div className="analysis-step"><span className="step-icon"><Sparkles size={20} /></span><div><h3>Understand the sound</h3><p>A local Audio Spectrogram Transformer listens to the first 10 seconds and suggests sound sources, categories, and descriptive tags.</p><span className="step-state neutral">AudioSet model · 527 sound classes · Runs on your CPU</span></div></div><div className="notice amber"><CircleHelp size={17} /><span>Sound identification is probabilistic, not ground truth. Results stay marked <strong>Needs review</strong>. Speech is classified, never transcribed.</span></div><label className="check-label rights"><input type="checkbox" checked={modelConsent} onChange={e => setModelConsent(e.target.checked)} disabled={analyzing} /><span>Download the model to my browser (about 90 MB, plus runtime files). Internet is needed on first use; cached files can be reused.</span></label>{analysisProgress && <div className="analysis-progress" role="status">{analyzing ? <LoaderCircle size={18} className="spin" /> : <CheckCircle2 size={18} />}<span>{analysisProgress}</span></div>}{analysisError && <div className="error-message">{analysisError}<br />Your signal measurements and files are unaffected.</div>}<footer className="modal-footer"><span><CloudOff size={14} />Your audio never leaves.</span><button className="button primary" disabled={!modelConsent || analyzing || !analysisIds.length} onClick={() => void identifySounds()}>{analyzing ? <LoaderCircle size={15} className="spin" /> : <Sparkles size={15} />}{analyzing ? 'Identifying…' : 'Identify sounds'}</button></footer></Modal>}
    ...
    {inspected && <SampleInspector key={inspected.id} sample={inspected} onSave={updateSample} onClose={closeInspector} onPlay={sample => void playSample(sample, true)} onIdentify={() => openAnalysis([inspected.id])} />}
